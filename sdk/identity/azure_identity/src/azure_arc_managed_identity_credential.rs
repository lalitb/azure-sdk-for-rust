...
impl AzureArcManagedIdentityCredential {
    ...
    pub fn new(
        ...
        let endpoint = Url::parse(&endpoint).with_context(ErrorKind::Credential, || {
            format!(
                "Azure Arc managed identity endpoint must be a valid URL, but is '{endpoint}'"
            )
        })?;
    ...
    async fn get_challenge_token(&self, resource: &str) -> azure_core::Result<String> {
        ...
        for attempt in 0..RETRY_ATTEMPTS {
            ...
            if attempt == RETRY_ATTEMPTS - 1 {
                ...
                let body = response.into_body().collect_string().await.unwrap_or_default();
                return Err(azure_core::Error::new(
                    ErrorKind::Credential,
                    format!(
    ...
    async fn request_token_with_challenge(
        ...
        request.insert_header(METADATA_HEADER.clone(), "true");
        request.insert_header(AUTHORIZATION_HEADER.clone(), format!("Basic {}", challenge_token));
        ...
        if !response.status().is_success() {
            ...
            return Err(azure_core::Error::new(
                ErrorKind::Credential,
                format!("Azure Arc token request failed. Status: {}, Body: {}", status, body)
            ));
        }
        ...
        let expires_on = if let Some(expires_on_str) = token_response["expires_on"].as_str() {
            expires_on_str.parse::<i64>().with_context(ErrorKind::Credential, || {
    ...
    async fn get_token(
        ...
        // Check cached token first
        {
            ...
            if let Some((token, _cached_at)) = cached.as_ref() {
                ...
                let expires_in = token.expires_on.unix_timestamp() - OffsetDateTime::now_utc().unix_timestamp();
                if expires_in > 300 {
        ...
        let access_token = self.request_token_with_challenge(resource, &challenge_token).await?;
...
mod tests {
    ...
    use azure_core::{
        http::{headers::{Headers, HeaderName}, Method, RawResponse, StatusCode, Url},
        Bytes,
    };
    ...
    async fn azure_arc_system_assigned() {
        ...
        let mock_client = MockHttpClient::new(move |req| {
            ...
            Box::pin(async move {
                ...
                assert_eq!(req.headers().get_str(&HeaderName::from_static("metadata")).unwrap(), "true");
                
                if count == 0 {
                    // First request: return challenge response
                    assert!(req.headers().get_str(&HeaderName::from_static("authorization")).is_err());
                    ...
                    assert_eq!(
                        req.headers().get_str(&HeaderName::from_static("authorization")).unwrap(),
    ...
    fn missing_imds_endpoint() {
        let result = AzureArcManagedIdentityCredential::new(
            ...
            TokenCredentialOptions {
                env: Env::from(&[
                    (ENDPOINT_ENV, "http://localhost:40342/metadata/identity/oauth2/token"),
                ][..]),
                ..Default::default()
            },
        );
